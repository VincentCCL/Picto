...
typedef struct fuzzyCovSent
{
        unsigned int sentId;
	string sent;
        unsigned int numWords;
        wordsCovered coverage; // bit string keeping track of words covered by shared ngrams
        double approxCovScore;
        bool queryDown;
        unsigned char partNum;
        unsigned int begParts[100]; // beginning / ending positions of matching parts
        unsigned int endParts[100];

        bool operator < (const fuzzyCovSent& s) const
        {
          return (approxCovScore < s.approxCovScore);
	}
}S_fuzzyCovSent;
...
public:
	...
	vector<S_SimplePhraseLocationElement> locateExactPhraseInCorpus(vector<IndexType> & phrase);
        ...
        vector<S_phraseLocationElement> findPhrasesInASentence(const char * srcSent,double minApproxCovScore,unsigned int nBest,vector<fuzzyCovSent> & bestFuzzySents);
        vector<S_phraseLocationElement> findPhrasesInASentence(vector<IndexType> & srcSentAsVocIDs,double minApproxCovScore,unsigned int nBest,vector<fuzzyCovSent> & bestFuzzySents);
